...
class DQN_Action_Masking:
    print("DQN_action_masking on")
    ...
    def main(cls):
        ...
        for n_epi in range(Parameters.r_param["episode"]):
            for dataid in Parameters.db_data:
                ...
                score = 0.0
                while not done:
                    a_list = q.sample_action_list(torch.from_numpy(s).float(), epsilon)

                    s_prime, r, done, a = env.step2(a_list, epsilon)
