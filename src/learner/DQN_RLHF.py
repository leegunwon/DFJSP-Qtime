...
class DQN_RLHF:
    ...
    def main(cls):
        ...
        for n_epi in range(Parameters.r_param["episode"]):
            ...
            epsilon = max(0.01, 0.08 - 0.002 * (n_epi / 200))
            s = env.reset()
            done = False
            score = 0.0
            while not done:
                ...
                if done == False:
                    memory.put((s, a, r, s_prime, done_mask, env.plotlydf))
                    s = s_prime
            ...
            makespan_list, q_over_time_list, score_list = cls.script_performance(env, n_epi, epsilon, memory, score,
                                                                                 True, makespan_list, q_over_time_list,
                                                                                 score_list)
    ...
    def get_result(cls, parameter):
        ...
        Flow_time, machine_util, util, makespan, tardiness, lateness, t_max, q_time_true, q_time_false, q_job_t, q_job_f, q_time, rtf = env.performance_measure()
        env.gantt_chart()
        Flow_time, machine_util, util, makespan, Tardiness_time, Lateness_time, T_max, q_time_true, q_time_false, q_job_t, q_job_f, total_q_time_over, rtf = env.performance_measure()
